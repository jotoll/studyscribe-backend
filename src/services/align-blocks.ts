import { GroqVerboseTranscription, DocBlock, BaseBlock } from '../types/docblocks-v2';

export function alignBlocksWithSegments(
  blocks: Array<Omit<DocBlock, 'time' | 'confidence' | 'speaker' | 'tags'>>,
  segments: GroqVerboseTranscription['segments']
): DocBlock[] {
  ...
  const segOffsets: Array<{startChar: number; endChar: number}> = [];
  let cursor = 0;
  ...
  function spanToTime(startChar: number, endChar: number) {
  ...
  function avgConfidence(segs: typeof segments) {
    const vals: number[] = [];
  ...
  return blocks.map(block => {
    ...
    const tryFind = (text: string) => {
      ...
      if (index >= 0) return [index, index + text.length] as const;
      return [-1, -1] as const;
    };
...
export function applyDiarizationToBlocks(
  blocks: DocBlock[],
  diarization: any // Placeholder para futuro DiarizationResult
): DocBlock[] {
