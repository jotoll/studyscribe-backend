...
const JSONEditor: React.FC<JSONEditorProps> = ({ initialData, onSave, onCancel, selectedPath, selectedElement }) => {
  ...
  const updateField = (path: (string | number)[], value: any) => {
    setEditedData((prev: any) => {
      // En modo single edit, editedData es la sección misma, no un documento completo
      // Si estamos editando una sección individual, el path debe ser relativo a la sección
      if (editMode === 'single' && selectedPath && selectedPath.startsWith('sections.')) {
        // Para secciones individuales, actualizar directamente en el objeto
        return {
          ...prev,
      ...
      // Modo completo: comportamiento normal
      const newData = { ...prev };
      let current = newData;
      ...
      for (let i = 0; i < path.length - 1; i++) {
        ...
        current = current[key];
        // Safety check to prevent accessing undefined
  ...
  const addExample = (sectionIndex: number) => {
    setEditedData((prev: any) => {
      ...
      if (!newSections[sectionIndex].examples) {
        newSections[sectionIndex].examples = [''];
      } else {
        newSections[sectionIndex].examples = [...newSections[sectionIndex].examples, ''];
      }
      return { ...prev, sections: newSections };
    });
  };
  ...
  const renderSingleSectionEditor = () => {
    ...
    if (selectedPath.startsWith('sections.')) {
      ...
      // Si editedData es la sección misma (modo single edit), usar editedData directamente
      // Si editedData tiene un array de sections (modo completo), buscar la sección por índice
    ...
    } else if (selectedPath === 'title') {
      // Para título, puede ser editedData.title (modo completo) o editedData directamente (modo single)
      const titleValue = editedData.title !== undefined ? editedData.title : editedData;
      if (titleValue === undefined) return null;
    ...
    } else if (selectedPath === 'summary') {
      // Para resumen, puede ser editedData.summary (modo completo) o editedData directamente (modo single)
      const summaryValue = editedData.summary !== undefined ? editedData.summary : editedData;
      if (summaryValue === undefined) return null;
        ...
        <View style={styles.singleEditContainer}>
          <View style={styles.sectionEditor}>
            ...
            <TextInput
              ...
              value={typeof summaryValue === 'string' ? summaryValue : ''}
